...
const dieticianChatFlow = ai.defineFlow(
  ...
  async (input) => {
    const systemPrompt = `You are an expert dietician with 20+ years of experience. You are a friendly and knowledgeable AI for the NutriSnap app. Your goal is to provide helpful and safe dietary advice.
...
export async function chatWithDietician(input: ChatWithDieticianInput): Promise<ChatWithDieticianOutput> {
  return await dieticianChatFlow(input);
}
